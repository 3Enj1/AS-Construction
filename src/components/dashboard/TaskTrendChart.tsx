...
export function TaskTrendChart({ data }: { data: { date: string; count: number }[] }) {
  ...
  return (
    <div className="as-card p-4 sm:p-5">
      ...
      {!hasActivity ? (
        <div className="mt-3 flex h-40 flex-col items-center justify-center gap-2 text-center">
          <div className="grid size-10 place-items-center rounded-full bg-muted text-muted-foreground">
            <TrendingUp className="size-5" />
          </div>
          <div className="text-sm font-medium">No tasks approved yet</div>
          <p className="max-w-xs text-xs text-muted-foreground">
            Approved tasks will start showing up here as a trend over the last 30 days.
          </p>
        </div>
      ) : (
