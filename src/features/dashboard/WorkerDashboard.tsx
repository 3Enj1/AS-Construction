...
import { CheckCircle2, Coffee, ClipboardList, Camera, MessageSquare, PlayCircle, StopCircle, Sandwich, Trophy } from "lucide-react";
  ...
  return (
    <>
      ...
      <div className="as-card relative overflow-hidden p-5">
        <div className="absolute inset-0 opacity-60" style={{ background: "linear-gradient(135deg, oklch(0.3 0.08 27 / 0.3), transparent 60%)" }} />
        <div className="relative">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Attendance</div>
              <div className="mt-0.5 flex items-center gap-2">
                <span className={"relative size-2.5 rounded-full " + (clock === "out" ? "bg-muted-foreground" : "bg-brand")}>
            ...
            {clock === "out" ? (
              <Button size="lg" className="h-14 px-6 bg-brand text-brand-foreground hover:bg-brand/90 as-press" onClick={() => updateClock("in", "Clocked in — have a great day")}>
                <PlayCircle className="size-5" /> Clock in
              </Button>
            ) : (
              <Button size="lg" variant="outline" className="h-14 px-6 as-press border-danger/40 text-danger hover:bg-danger/10" onClick={() => updateClock("out", "Clocked out — see you tomorrow")}>
                <StopCircle className="size-5" /> Clock out
              </Button>
            )}
          </div>
          <div className="mt-4 grid grid-cols-2 gap-2">
            <Button variant="outline" disabled={clock === "out"} onClick={() => updateClock("tea", "Tea break started")} className="h-11 as-press">
              <Coffee className="size-4" /> Tea break
            </Button>
            <Button variant="outline" disabled={clock === "out"} onClick={() => updateClock("lunch", "Lunch break started")} className="h-11 as-press">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        ...
        <div className="as-card p-3 text-center">
          ...
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Done today</div>
        </div>
        <div className="as-card p-3 text-center">
          <div className={"text-2xl font-semibold " + (overdue ? "text-danger" : "")}>{overdue}</div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        <Button variant="outline" asChild className="h-16 flex-col gap-1 as-press">
          <Link to="/uploads"><Camera className="size-5 text-brand" /><span className="text-[11px]">Upload photo</span></Link>
        </Button>
        <Button variant="outline" asChild className="h-16 flex-col gap-1 as-press">
          <Link to="/chat"><MessageSquare className="size-5 text-info" /><span className="text-[11px]">Project chat</span></Link>
        </Button>
        <Button variant="outline" asChild className="h-16 flex-col gap-1 as-press">
          <Link to="/performance"><Trophy className="size-5 text-warning" /><span className="text-[11px]">Performance</span></Link>
        </Button>
      </div>
      ...
      <h2 className="mt-6 mb-2 flex items-center justify-between text-sm font-semibold uppercase tracking-wider text-muted-foreground">
        <span><ClipboardList className="inline size-4 mr-1" /> Today's tasks ({pending})</span>
        <Link to="/tasks" className="text-xs font-medium normal-case tracking-normal text-brand hover:underline">See all →</Link>
      </h2>
      ...
      {isLoading && (
        <div className="as-card p-4 text-sm text-muted-foreground">Loading tasks…</div>
      )}
    </>
  );
}
