...
export function ManagerDashboard() {
  const { data: tasks = [] } = useQuery({ queryKey: ["tasks", "all"], queryFn: () => fetchEnrichedTasks() });
  const { data: activeCount = 0 } = useQuery({
    queryKey: ["projects-active-count"],
    queryFn: async () => {
      const { count } = await supabase.from("projects").select("id", { count: "exact", head: true }).eq("is_archived", false).neq("status", "completed");
      return count ?? 0;
    },
  });
  ...
  return (
    <>
      ...
      <h2 className="mt-10 mb-3 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Awaiting your approval</h2>
      <div className="grid gap-3 md:grid-cols-2">
        {review.map((t) => <TaskCard key={t.id} task={t} />)}
        {review.length === 0 && <div className="as-card p-6 text-sm text-muted-foreground">Nothing waiting.</div>}
      </div>
    </>
  );
}
