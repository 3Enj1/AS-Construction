...
export function SupervisorDashboard() {
  ...
  const active = tasks.filter(
    (t) => !["approved", "archived"].includes(t.dbStatus),
  );
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard label="Awaiting review" value={awaitingReview.length} icon={CheckCircle2} tone="warning" />
      </div>
      <h2 className="mt-8 mb-3 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Today's tasks</h2>
      {isLoading ? (
