...
import { fetchAttendanceLogs, fetchProjectsMini } from "@/lib/project-actions";
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Clock-ins, breaks and hours over the last 7 days."
      />
      <div className="as-card overflow-hidden">
        {isLoading ? (
          <div className="p-6 text-sm text-muted-foreground">Loading attendance…</div>
        ) : attendance.length === 0 ? (
          <div className="p-6 text-sm text-muted-foreground">No attendance logged yet.</div>
        ) : (
              ...
              {attendance.map((a) => {
                ...
                return (
                  <tr key={a.id} className="border-t border-border">
                    ...
                    <td className="p-3 text-muted-foreground">{p?.project_name ?? "—"}</td>
